...
class FixedPerLot(Commission):
    """A :class:`Commission` class that charges a fix commission per lot.

    :param lot_commission: The percentage to charge.
    :type lot_commission: float.
    """
    def __init__(self, lot_commission):
        # super(TradePercentage, self).__init__()
        assert(lot_commission > 0)
        self.__lot_commission = lot_commission

    def calculate(self, order, price, quantity):
        return quantity * self.__lot_commission
...
class FutureBroker(broker.Broker):
    """Backtesting broker.
